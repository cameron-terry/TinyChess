...
class Board {
    ...
    getMaterial() {
        ...
        if (this.side == COLORS.WHITE || this.side == COLORS.BLACK) {
            for (var i = 0; i < this.area.length; i++) {
                if (this.area[i].color == this.side) {
                ...
                } else if (this.area[i].color == this.side^1) {
                    switch (this.area[i].piece) {
                        case PIECES.PAWN:
                            score -= SCORES.PAWN;
                            break;
            ...
            throw "Err counting pieces for side.";
        }
    ...
    createMoves (sq, piece) {
        ...
        switch (piece) {
            ...
            case PIECES.BISHOP:
                ...
                break;
            case PIECES.PAWN:
                var sq_is_on_white_start = (sq == SQUARES.A2 || sq == SQUARES.B2 || sq == SQUARES.C2 || sq == SQUARES.D2) && (this.side == COLORS.WHITE);
                var sq_is_on_black_start = (sq == SQUARES.A5 || sq == SQUARES.B5 || sq == SQUARES.C5 || sq == SQUARES.D5) && (this.side == COLORS.BLACK);
    ...
    generateMoveList() {
                    ...
                    if (move.length > 0) {
                        for (var j = 0; j < move.length; j++) {
                            ...
                            if (!from.offboard && !to.offboard) {
                                ...
                                if (pce_moved) {
                                    takeMove(move[j][0], move[j][1], from_pce, to_pce, from_color, to_color, this);
        ...
        // check for castle perms
        var canCastle = this.checkCastle();
        if (canCastle) {
            if (this.side == COLORS.WHITE) {
                move_list.push([SQUARES.D1, SQUARES.B1]);
            } else {
                move_list.push([SQUARES.D6, SQUARES.B6]);
            }
        }
    ...
    checkCastle() {
        if (this.side == COLORS.WHITE) {
            ...
            && (this.area[SQUARES.B1].piece == PIECES.EMPTY && this.area[SQUARES.C1].piece == PIECES.EMPTY) && this.whiteCastle && !this.inCheck()) {
               return true;
            }
        } else {
            ...
            && (this.area[SQUARES.B6].piece == PIECES.EMPTY && this.area[SQUARES.C6].piece == PIECES.EMPTY) && this.blackCastle && !this.inCheck()) {
                return true;
            }
        }
    ...
    isMate() {
        ...
        if (this.side == COLORS.BLACK) {
            ...
            if (this.inCheck()) {
                ...
                for (var j = 0; j < this.area.length; j++) {
                    found_black_piece = (this.area[j].piece != PIECES.EMPTY && this.area[j].color == COLORS.BLACK);
                        ...
                        if (square_safe == -1) {
                            ...
                            if (piece_moved) {
                                if (!this.inCheck()) {
                                    ...
                                    takeMove(king_sq, king_to, PIECES.KING, to_pce, COLORS.WHITE, to_color, this);
                                    break;
                                }
                ...
                if (!king_can_move) {
                    ...
                    for (var k = 0; k < this.area.length; k++) {
                        found_white_piece = (this.area[k].piece != PIECES.EMPTY && this.area[k].color == COLORS.WHITE && this.area[k].piece != PIECES.KING);
                                    ...
                                    if (attacking_piece_sqs.indexOf(defending_square) != -1) {
                                        ...
                                        if (piece_moved) {
                                            if (!this.inCheck()) {
                                                ...
                                                takeMove(k, defending_square, from_pce, PIECES.EMPTY, from_color, COLORS.NONE, this);
                                                break;
                                            }
            ...
            if (this.inCheck()) {
                ...
                var found_white_piece = false;
                for (var j = 0; j < this.area.length; j++) {
                    found_white_piece = (this.area[j].piece != PIECES.EMPTY && this.area[j].color == COLORS.WHITE);
                            ...
                            if (to_color != COLORS.BLACK) {
                                ...
                                if (piece_moved) {
                                    if (!this.inCheck()) {
                                        ...
                                        takeMove(king_sq, king_to, PIECES.KING, to_pce, COLORS.BLACK, to_color, this);
                                        break;
                                    }
                                    takeMove(king_sq, king_to, PIECES.KING, to_pce, COLORS.BLACK, to_color, this);
                                }
                ...
                if (!king_can_move) {
                    ...
                    for (var k = 0; k < this.area.length; k++) {
                        found_black_piece = (this.area[k].piece != PIECES.EMPTY && this.area[k].color == COLORS.BLACK && this.area[k].piece != PIECES.KING);
                        if (found_black_piece) {
                            // move to see if piece can block the attack
                            p_move = this.createMoves(k, this.area[k].piece);
    ...
    checkState() {
        ...
        if (!found_state) {
            ...
            if (this.gameOver) {
                if (this.side == COLORS.WHITE) {
                    ...
                    for (var i = 0; i < this.area.length; i++) {
                        found_king = this.area[i].piece == PIECES.KING && this.area[i].color == COLORS.BLACK;
                    ...
                    playSound('end');
                    setTimeout(function() { alert("Game is over [White wins by checkmate]"); $(sq_name).attr("src","img/bK_check.png"); }, 1000);
                } else {
                    var found_king = false;
                    for (var i = 0; i < this.area.length; i++) {
                        found_king = this.area[i].piece == PIECES.KING && this.area[i].color == COLORS.WHITE;
                    ...
                    playSound('end');
                    setTimeout(function() { alert("Game is over [Black wins by checkmate]"); $(sq_name).attr("src","img/wK_check.png"); }, 1500);
                }
        ...
        if (!found_state) {
            ...
            if (this.gameOver) {
                ...
                playSound('end');
                setTimeout(function() { alert('game is a draw [insufficient material]'); }, 1500);
        ...
        if (!found_state) {
            ...
            if (this.gameOver) { found_state = true; this.side = -1; }
        }
        ...
        if (!found_state) {
            ...
            if (this.move_list.length > 5) {
                ...
                var three_fold = true;
                for (var i = 0; i < 3; i++) {
                    if (this.move_list[moves_end - i] != this.move_list[moves_end - (i + 4)]) {
                        three_fold = false;
                    }
                }
            }
            ...
            if (three_fold) {
                ...
                playSound('end');
                setTimeout(function() { alert('game is a draw [three-fold repetition]'); }, 1500);
